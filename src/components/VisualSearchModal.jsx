...
// ✅ PROMPT AMÉLIORÉ POUR L'INTELLIGENCE VISUELLE
const IDENTIFY_PROMPT = `Tu es un expert en Vision par Ordinateur de classe mondiale. 
Analyse cette image (même si elle est floue ou partielle) pour en extraire l'ADN sémantique.

TA MISSION :
1. DÉDUIRE L'IDENTITÉ : Si l'image est floue, analyse la silhouette. Une forme allongée jaune = Banane. Des cercles sur un arceau = Casque Audio.
2. EXTRAIRE LES "ANCHOR POINTS" : Repère les détails immuables (positions des logos, texture, type de connectique, motifs de semelles).
3. DÉFINIR LE CONTEXTE : Est-ce un produit fini, une partie d'un tout (ex: une main de banane pour un régime), ou un accessoire ?

RETOURNE UNIQUEMENT CE JSON :
{
  "category": "Audio Lab | Clothing | Shoes | Tech Lab | Fragrance | Accessories | Food",
  "conceptKey": "Le mot-clé racine (ex: 'Banane', 'AirPods', 'Jordan')",
  "visualSignature": "Description technique ultra-précise des détails (ex: 'boîtier blanc brillant, tige courte, capteur noir latéral')",
  "searchQuery": "Requête optimisée pour base de données",
  "reasoning": "Logique humaine derrière l'identification"
}`;
...
const matchProducts = (products, analysis) => {
  ...
  const scored = products.map((p) => {
    ...
    const searchable = normalize(
      `${p.name} ${p.type} ${p.brand || ""} ${(p.features || []).join(" ")}`
    );
    let score = 0;
...
const VisualSearchModal = ({ isOpen, onClose, addToCart }) => {
  ...
  const searchByImage = async () => {
    ...
    try {
      ...
      // ── PASSE 1 : Extraction d'ADN Visuel ──
      setLoadingStep("Étape 1/2 — Analyse de l'empreinte...");
      const identification = await callGemini({
        type: "image",
        prompt: IDENTIFY_PROMPT, // S'assurer que le prompt demande "confidenceScore"
        imageBase64,
      ...
      setLoadingStep("Scan du catalogue OneFreeStyle...");
      
      // On récupère TOUS les produits de la catégorie pour ne rien rater
      let { data: allProducts, error: supaError } = await supabase
        .from("products")
        .select("*")
        .eq("type", identification.type);
      ...
      // ✅ ALGORITHME DE MATCHING ÉLITE (Scoring Dynamique)
      const concept = (identification.conceptKey || identification.productName || "").toLowerCase();
      ...
      const candidates = allProducts
        .map(p => {
          ...
          // 1. Match Exact ou Conceptuel (Poids lourd)
          // Si le nom du produit contient le mot clé principal identifié
          if (concept && name.includes(concept.split(' ')[0])) score += 100;
          
          // 2. Match sur les caractéristiques visuelles (Couleur, Matière)
          if (identification.visualFeatures) {
            identification.visualFeatures.forEach(feat => {
              const f = feat.toLowerCase();
              if (name.includes(f)) score += 10;
              if (desc.includes(f)) score += 5;
            });
          }
  
          // 3. Sécurité : Si l'image est identique (cas où l'URL est connue par l'IA)
          // (Optionnel selon ton catalogue)
          
          return { ...p, _tempScore: score };
        })
        .sort((a, b) => b._tempScore - a._tempScore)
        .slice(0, 10); // On garde les 10 meilleurs pour la Passe 2
  
      // ── PASSE 2 : Verrouillage visuel (Le Match à 100%) ──
      setLoadingStep(`Étape 2/2 — Comparaison de précision...`);
      ...
      // On force le résultat de Gemini en tête
      const ranked = rankedIndices
        .map((i) => candidates[i])
        .filter((p) => p && !seenIds.has(p.id));
  
      ranked.forEach(p => seenIds.add(p.id));
  
      // On ajoute les restes du scoring sémantique au cas où
      const leftovers = candidates.filter((p) => !seenIds.has(p.id));
  
      const finalResults = [...ranked, ...leftovers].slice(0, 8);
      setResults(finalResults);
  
      // ✅ LOGIQUE DE VALIDATION DU MATCH 100%
      if (comparison.reasoning) {
        setAiAnalysis((prev) => ({
          ...prev,
          comparisonReasoning: comparison.reasoning,
          // On affiche un badge "Authentifié" si le match est parfait
          isPerfectMatch: comparison.topMatch !== undefined && finalResults.length > 0
        }));
      }
  
    } catch (err) {
      console.error("Critical Visual Search Failure:", err);
  ...
  const searchByDescription = async () => {
    ...
    try {
      ...
      if (analysis.type && primaryTerm) {
        query = query.or(
          `type.eq.${analysis.type},name.ilike.%${primaryTerm}%`
        );
    ...
    <div className="fixed inset-0 z-[350] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-zinc-950 border border-white/10 rounded-t-3xl sm:rounded-3xl w-full sm:max-w-3xl max-h-[92vh] flex flex-col shadow-[0_0_80px_rgba(0,255,136,0.08)]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/5 flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-zinc-400 hover:text-white transition-all"
          >
            ...
            <>
                ...
                {imagePreview ? (
                  <div className="relative group">
                    <img
                      src={imagePreview}
                      alt="Preview"
                      className="w-full h-48 object-contain bg-black/40 p-2"
                    />
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    >
                      <span className="bg-primary text-black text-[10px] font-black uppercase px-4 py-2 rounded-xl">
                        Changer l'image
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="py-12 text-center px-6">
                    ...
                    <p className="font-black uppercase text-zinc-300 text-sm mb-1">
                      Glissez une photo produit
                    </p>
              ...
              {!imagePreview && (
                <div className="flex items-start gap-3 bg-primary/5 border border-primary/15 rounded-xl p-3">
                  ...
                  <p className="text-[10px] font-bold text-zinc-400">
                    Gemini{" "}
                    <span className="text-primary">compare visuellement</span>{" "}
                    ton image avec chaque photo produit du catalogue — pas de
                    mots-clés, une vraie comparaison d'images.
                  </p>
                </div>
              )}
                ...
                <div className="bg-primary/5 border border-primary/20 rounded-xl p-4">
                  ...
                  <div className="mt-3 h-1 bg-white/5 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: loadingStep.includes("2/2") ? "80%" : "35%",
                      }}
                    ></div>
                  </div>
                </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    <i className="fa-solid fa-eye text-xs"></i> Comparer
                    visuellement
                  </>
                )}
              </button>
          ...
          {mode === "text" && (
            <>
              <div>
                ...
                />
                <p className="text-[9px] font-bold text-zinc-600 mt-1.5 text-right">
                  Entrée pour rechercher
                </p>
              </div>
              <div>
                <p className="text-[9px] font-black uppercase text-zinc-600 tracking-widest mb-2">
                  Essayez :
                </p>
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4">
              ...
              <div className="space-y-2">
                {aiAnalysis.reasoning && (
                  <p className="text-zinc-300 text-[11px] font-bold leading-relaxed">
                    {aiAnalysis.reasoning}
                  </p>
                )}
            ...
            <div>
              ...
              {results.length === 0 ? (
                <div className="py-12 text-center border border-white/5 rounded-2xl">
                  ...
                  <p className="font-black uppercase italic text-zinc-500 text-sm">
                  ...
                  {results.map((product, idx) => (
                    ...
                    >
                      <div className="aspect-square overflow-hidden bg-zinc-900 relative">
                        ...
                        {mode === "image" && idx > 0 && (
                          <div className="absolute top-2 left-2">
                            <span className="bg-black/60 text-zinc-300 text-[8px] font-black px-2 py-0.5 rounded-full">
                              #{idx + 1}
                            </span>
                          </div>
                        )}
                        {mode === "text" && (
                          <div className="absolute top-2 left-2">
                            <span className="bg-primary text-black text-[7px] font-black px-2 py-0.5 rounded-full uppercase">
                              {product.status}
                            </span>
                          </div>
                        )}
                      ...
                      <div className="p-2.5">
                        <p className="font-black uppercase italic text-white text-[10px] truncate">
                          {product.name}
                        </p>
                        <p className="text-[9px] text-zinc-600 font-black uppercase mt-0.5">
                          {product.type}
                        </p>
