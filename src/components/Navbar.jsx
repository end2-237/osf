...
const CATEGORIES = [
  { name: 'Audio',         shortName: 'Audio',   icon: 'fa-headphones',         color: 'text-primary',    subs: ['Audio Lab', 'Casques & Écouteurs', 'Enceintes', 'AirPods', 'Wireless'] },
  { name: 'Tech Lab',      shortName: 'Tech',    icon: 'fa-microchip',           color: 'text-blue-400',   subs: ['VR & AR', 'Accessoires', 'Gadgets', 'Smart Watch', 'Cables'] },
  { name: 'Streetwear',    shortName: 'Street',  icon: 'fa-shirt',               color: 'text-purple-400', subs: ['T-Shirts', 'Hoodies', 'Vestes', 'Shorts', 'Accessoires'] },
  { name: 'Sneakers',      shortName: 'Shoes',   icon: 'fa-shoe-prints',         color: 'text-orange-400', subs: ['Baskets', 'Running', 'Casual', 'High-Top', 'Limited'] },
  { name: 'Parfums',       shortName: 'Parfum',  icon: 'fa-spray-can-sparkles',  color: 'text-pink-400',   subs: ['Homme', 'Femme', 'Unisex', 'Travel Size', 'Gift Set'] },
  { name: 'Flash Deals',   shortName: 'Deals',   icon: 'fa-bolt',                color: 'text-yellow-400', subs: ['Moins de 10K', 'Moins de 25K', 'Meilleures Ventes', 'Nouveautés'] },
];
...
const Navbar = ({ isDark, toggleTheme, cartCount, toggleCart }) => {
  ...
  return (
    <>
      ...
      <nav className="fixed left-0 right-0 z-[110] bg-black border-b border-white/5 top-8 md:top-10">
        ...
        <div className="h-12 md:h-16 flex items-center gap-2 md:gap-4 px-3 sm:px-4 md:px-8 max-w-[1600px] mx-auto">
          ...
          <Link to="/" className="flex items-center gap-2 flex-shrink-0 group">
            <img src={ofsLogo} alt="OFS" className="w-7 h-7 md:w-9 md:h-9 flex-shrink-0 drop-shadow-[0_0_8px_rgba(0,255,136,0.35)] group-hover:scale-110 transition-transform" />
          ...
          <form onSubmit={handleSearch} className="hidden md:flex flex-grow max-w-2xl">
            <div className={'flex items-center w-full bg-zinc-900 border-2 rounded-xl overflow-hidden transition-all duration-300 ' + (searchFocused ? 'border-primary shadow-[0_0_16px_rgba(0,255,136,0.2)]' : 'border-white/10 hover:border-white/20')}>
              ...
              />
              {searchQuery && <button type="button" onClick={() => setSearchQuery('')} className="text-zinc-500 hover:text-white pr-2 transition"><i className="fa-solid fa-xmark text-xs"></i></button>}
              <button type="submit" className="bg-primary text-black px-4 lg:px-6 py-[10px] font-black text-[9px] lg:text-[10px] uppercase tracking-widest hover:bg-white transition-colors flex-shrink-0 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 flex-shrink-0">
            {/* Theme */}
            <button onClick={toggleTheme} className="w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:border-primary/50 transition-all">
              <i className={'fa-solid text-xs ' + (isDark ? 'fa-sun text-yellow-400' : 'fa-moon text-white')}></i>
            </button>

            {/* Cart */}
            <button onClick={toggleCart} className={'relative w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-xl transition-all duration-200 ' + (cartCount > 0 ? 'bg-primary text-black' : 'bg-white/5 text-white border border-white/10 hover:border-primary/50') + (cartBump ? ' scale-125' : ' scale-100')}>
            ...
            {/* Vendre */}
            <Link to="/admin" className="hidden sm:flex items-center gap-1.5 bg-primary text-black px-3 md:px-4 py-2 md:py-2.5 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-wide hover:bg-white transition-all whitespace-nowrap">
              <i className="fa-solid fa-bolt text-xs"></i>
              <span>Vendre</span>
            </Link>

            {/* Hamburger */}
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="xl:hidden w-8 h-8 md:w-9 md:h-9 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 hover:border-primary/40 transition-all">
        ...
        <div className="md:hidden px-3 sm:px-4 pb-2">
          <form onSubmit={handleSearch} className="flex items-center bg-zinc-900 border border-white/10 rounded-xl overflow-hidden focus-within:border-primary transition-colors">
            ...
            />
            {searchQuery && <button type="button" onClick={() => setSearchQuery('')} className="px-2 text-zinc-500 hover:text-white transition flex-shrink-0"><i className="fa-solid fa-xmark text-xs"></i></button>}
            <button type="submit" className="bg-primary text-black px-4 py-2.5 font-black uppercase flex-shrink-0 flex items-center">
      ...
      <div id="ofs-catbar" className="fixed left-0 right-0 z-[105] bg-zinc-950 border-b border-white/5" onMouseLeave={handleMegaLeave}>
        <div className="max-w-[1600px] mx-auto px-3 md:px-8">
          <div className="flex items-center overflow-x-auto hide-scrollbar">
            ...
            <button onClick={() => navigate('/store')} onMouseEnter={() => handleCategoryHover(-1)} className="flex items-center gap-1.5 px-2.5 sm:px-3 md:px-4 py-2.5 md:py-3 font-black uppercase text-[8px] sm:text-[9px] md:text-[10px] tracking-wide text-white hover:text-primary transition-colors border-r border-white/5 flex-shrink-0">
            ...
            {CATEGORIES.map((cat, idx) => (
              <button
                ...
                className={'flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 md:px-4 py-2.5 md:py-3 font-black uppercase text-[8px] sm:text-[9px] md:text-[10px] tracking-wide border-r border-white/5 flex-shrink-0 whitespace-nowrap transition-all ' + (activeCategory === idx && megaMenuOpen ? 'text-primary bg-white/5' : 'text-zinc-400 hover:text-white hover:bg-white/5')}
              >
            ...
            <Link to="/studio" className="flex items-center gap-1.5 px-2.5 sm:px-3 md:px-4 py-2.5 md:py-3 font-black uppercase text-[8px] sm:text-[9px] md:text-[10px] tracking-wide text-primary hover:bg-primary/10 transition-all flex-shrink-0 ml-auto whitespace-nowrap">
              ...
              <div className="grid grid-cols-5 gap-8">
                <div className="col-span-1">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <h3 className="font-black text-white uppercase text-sm">{CATEGORIES[activeCategory].name}</h3>
                      <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">OneFreestyle Elite</p>
                    </div>
                  </div>
                  <Link to="/store" className="w-full bg-primary text-black text-[10px] font-black uppercase tracking-widest py-3 px-4 rounded-xl flex items-center justify-between hover:bg-white transition-colors">
                ...
                <div className="col-span-2">
                  <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 rounded-2xl p-6 h-full flex flex-col justify-between">
                    ...
                    <Link to="/store" className="text-[10px] font-black uppercase text-black bg-primary px-4 py-2.5 rounded-xl hover:bg-white transition-colors w-fit">Profiter</Link>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* MOBILE DRAWER */}
      <div className={'xl:hidden fixed right-0 top-0 bottom-0 z-[210] bg-zinc-950 border-l border-white/10 flex flex-col shadow-2xl transition-transform duration-300 ' + (mobileMenuOpen ? 'translate-x-0' : 'translate-x-full')} style={{width: 'min(320px, 85vw)'}}>
        ...
        <div className="flex-shrink-0 border-t border-white/5 p-4 space-y-2 bg-black/50">
          <Link to="/admin" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-center gap-2 bg-primary text-black p-3.5 rounded-2xl font-black uppercase text-[10px] tracking-widest w-full hover:bg-white transition-all active:scale-95">
            <i className="fa-solid fa-bolt text-xs"></i><span>Devenir Vendeur</span>
          </Link>
          <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="flex items-center justify-center gap-2 border border-white/10 text-zinc-400 p-3 rounded-2xl font-black uppercase text-[9px] tracking-widest w-full hover:border-primary/40 hover:text-white transition-all">
            <i className="fa-solid fa-right-to-bracket text-xs"></i><span>Espace Vendeur</span>
          </Link>
        </div>
      </div>
