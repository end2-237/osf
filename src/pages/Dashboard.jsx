...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex text-gray-900">
      ...
      <aside className={`fixed lg:sticky lg:top-0 lg:bottom-auto z-40 inset-y-0 left-0 lg:h-screen w-[224px] flex-shrink-0 bg-white border-r border-gray-200 flex flex-col transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        {/* L'identité de la boutique, pas un logo de plateforme : le vendeur
            doit reconnaître son espace au premier coup d'œil. */}
        <div className="px-3 pt-3.5 pb-2.5 flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-bold text-[12.5px] truncate leading-tight">{vendor.shop_name}</p>
          ...
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden w-7 h-7 rounded-lg hover:bg-gray-100 text-gray-400 flex-shrink-0 text-[12px]">
        ...
        <div className="px-3 pb-2.5">
          <div className="relative">
            ...
            <input value={navQuery} onChange={e => setNavQuery(e.target.value)}
              ...
              className="w-full bg-gray-100 rounded-lg pl-7 pr-2.5 py-1.5 text-[11px] outline-none focus:ring-2 focus:ring-orange-500/20" />
          </div>
        </div>
          ...
          {NAV_GROUPS.map(group => {
            ...
            return (
              <div key={group.title} className="mb-2.5 last:mb-0">
                <p className="px-2.5 mb-1 text-[9px] font-bold uppercase tracking-[0.1em] text-gray-400">
                  {group.title}
                </p>
                  ...
                  {items.map(n => {
                    ...
                    return (
                      <button key={n.key} onClick={() => go(n.key)}
                        className={`w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-lg text-[12px] font-semibold transition-colors ${
                          active ? "bg-orange-500 text-white shadow-sm shadow-orange-500/25"
                                 : lock ? "text-gray-400 hover:bg-gray-50"
                                        : "text-gray-600 hover:bg-gray-100"}`}>
                        <i className={`fa-solid ${n.icon} w-3.5 text-center text-[11px] ${active ? "" : lock ? "text-gray-300" : "text-gray-400"}`} />
                        <span className="flex-1 text-left truncate">{n.label}</span>
                        {lock
                          ? <i className={`fa-solid fa-lock text-[9px] ${active ? "text-white/70" : "text-gray-300"}`} />
                          : n.badge > 0 && (
                            <span className={`text-[9px] font-bold min-w-[17px] h-[17px] px-1 rounded-full flex items-center justify-center ${active ? "bg-white/25 text-white" : "bg-orange-500 text-white"}`}>
                              {n.badge}
                            </span>
                          )}
                      </button>
        ...
        <div className="px-2 pb-1.5">
          {expired ? (
            <div className="rounded-xl bg-red-50 border border-red-100 p-2.5">
              <p className="text-[11px] font-bold text-red-700">Abonnement expiré</p>
              <p className="text-[10px] text-red-600/80 mt-0.5 leading-snug">
                Ta boutique n'est plus visible sur Buyticle.
              </p>
              <button onClick={() => go("abonnement")}
                className="mt-2 w-full bg-red-600 text-white text-[10.5px] font-bold py-1.5 rounded-lg hover:bg-red-700">
                Renouveler
              </button>
            </div>
          ) : plan?.allows_sponsored ? (
            <div className="rounded-xl bg-gray-900 text-white p-2.5">
              <p className="text-[11px] font-bold">Forfait {plan.plan_name}</p>
            ...
            <div className="rounded-xl bg-gray-900 text-white p-2.5 relative overflow-hidden">
              <div className="absolute -right-5 -top-5 w-16 h-16 rounded-full bg-orange-500/30 blur-xl" />
              <div className="relative">
                <div className="w-6 h-6 rounded-lg bg-orange-500 flex items-center justify-center mb-1.5">
                  <i className="fa-solid fa-bolt text-[10px]" />
                </div>
                <p className="text-[11px] font-bold leading-tight">Passe au forfait supérieur</p>
                <p className="text-[9.5px] text-white/55 mt-0.5 leading-snug">
                ...
                <button onClick={() => go("abonnement")}
                  className="mt-2 w-full bg-orange-500 text-white text-[10.5px] font-bold py-1.5 rounded-lg hover:bg-orange-600">
                  Voir les forfaits
                </button>
              </div>
            </div>
        ...
        <div className="p-2 pt-1.5 border-t border-gray-100 space-y-0.5">
          <a href={`/shop/${vendor.shop_name}`} className="w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-lg text-[12px] font-semibold text-gray-600 hover:bg-gray-100"><i className="fa-solid fa-arrow-up-right-from-square w-3.5 text-center text-[11px] text-gray-400" />Ma boutique</a>
          <button onClick={() => { signOut(); navigate("/"); }} className="w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-lg text-[12px] font-semibold text-red-500 hover:bg-red-50"><i className="fa-solid fa-right-from-bracket w-3.5 text-center text-[11px]" />Déconnexion</button>
        </div>
      </aside>
